...
public class PruebasStreams {
	...
	public static void main(String[] args) {
		...
		// Los streams solo pueden recorrerse una vez, ya que estan diseñados para consumirse/cerrarse despues de su recorrido.
		//long n2 = s1.count();
		...
		System.out.println("==========================CONCATENANDO FILTROS==========================");
		int id = 3;
		facturas.stream()
		...
		// Cada objeto del stream es procesado uno por uno en cada uno d elos pasos antes de pasar al siguiente objeto
		//Los objetos NO se procesan en grupo por cada paso, sino individualmente hasta llegar al ultimo paso

		// int id = 3;
		System.out.println("=======================================");
		facturas.stream()
				.filter(fra -> {
					System.out.println("Paso 1: filtra factura con total>130" + fra);
					return fra.getTotal() > 130;
				})
				.filter(fra -> {
					System.out.println("Cumple paso1 y llega a Paso 2: filtra que factura tenga el id dado" + fra);
					return fra.getCliente().getId() == id;
				})
				.forEach(fra -> System.out.println("cumple paso 2 y llega a Paso 3: imprime factura" + fra));
		...
		//Podemos utilizar el iterados para obtener unicamente los objetos finales que seran obtenidos despues de cada paso
		while (it.hasNext()) {
		...
		//
		// Skip: Se salta cierto numero de elementos al inicio del stream
		//
		...
		// devuelva un stream de tipo distinto....aunque en este ejemplo si esta regresando stream d eun tipo distinto, nonsense
		palabras.stream().map(str -> new Factura(str)).forEach(fac -> System.out.println("codigo: "+fac));
		...
		/*
		Collectors nos mandara todos los objetos salidos del stream a una lista o mapa, segun lo necesitemos
		* */
		...
		//
		// Join (agregacion): obtenemos un string a partir del stream, puede usarse para concatenacion
		...
		/*
		// Flat map
		Lo utilizamos cuando queremos aplanar varias colecciones de datos a una unica coleccion de datos
		Por ejemplo, varias listas a una unica lista conteniendo todos lso elementos de las listas anteriores
		*/
		...
		// Sin flat map
		/*Aqui tenemos la lista clientes list<Cliente>, de la cual obtenemos la lista de facturas List<Factura> que tiene cada cliente
		para posteriormente añadir cada una de esas listas a una unica lista --> facturitas
		*/
		...
		// Mejor con flat map:
		/*Aqui tenemos la lista clientes list<Cliente>, de la cual obtenemos el stream factura spor medio de c.getFacturas para cada cliente
		para posteriormente obtener una lista unica por medio de flatMap y collect
		*/

		List<Factura> facturitas2 = clientes.stream()
